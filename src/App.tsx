...
export default function App() {
  ...
  const toggleGroup = (categoryId: string) => {
    if (activeColorPicker) return; // Prevent collapse when interacting with color picker
                              ...
                              <>
                                ...
                                {tabs.filter(t => t.pinned).length > 0 && (
                                  <div className="bg-white/[0.02] group/pinned border-b border-white/[0.03]">
                                    <div 
                                      className="px-4 py-2.5 flex items-center justify-between hover:bg-white/[0.03] cursor-pointer transition-colors"
                                    ...
                                    <div key={categoryId} className="group/group">
                                      ...
                                      >
                                        <div className="flex items-center gap-2 flex-1 min-w-0">
                                          <div className={`flex items-center justify-center w-6 h-6 rounded-lg shadow-inner border border-white/5 shrink-0`} style={{ backgroundColor: uiColor.hex + '33', color: uiColor.hex }}>
                                            {React.cloneElement(getCategoryIcon(organizedGroups[categoryId]?.title || categoryId), { size: 12 })}
                                          </div>
                                          ...
                                          {organizedGroups[categoryId] ? (
                                            <input 
                                              ...
                                              className="bg-transparent border-none text-xs font-bold tracking-wider text-slate-300 focus:outline-none focus:text-white transition-colors min-w-[50px] w-auto max-w-[120px] truncate"
                                              placeholder="Group"
                                            />
                                          ) : (
                                            <span className="text-xs font-bold tracking-wider text-slate-300 truncate">{categoryId}</span>
                                          )}
                                        ...
                                        <div 
                                          className="flex items-center gap-1.5 transition-all px-2 relative"
                                          onClick={(e) => e.stopPropagation()}
                                        >
                                          {organizedGroups[categoryId] && (
                                            <>
                                              {activeColorPicker === categoryId ? (
                                                <div className="flex gap-1.5 bg-slate-900 border border-white/10 shadow-xl px-2 py-1.5 rounded-full absolute right-8 top-1/2 -translate-y-1/2 z-50">
                                                   <button onClick={() => setActiveColorPicker(null)} className="mr-1 text-slate-500 hover:text-white flex items-center justify-center shrink-0"><X size={12} /></button>
                                                   {CHROME_COLORS.map(c => (
                                                      <button
                                                        key={c}
                                                        onClick={() => {
                                                          handleUpdateGroup(categoryId, organizedGroups[categoryId].title, c);
                                                          setActiveColorPicker(null);
                                                        }}
                                                        className={`w-3 h-3 rounded-full border shrink-0 ${organizedGroups[categoryId].color === c ? 'border-white scale-110 shadow-sm shadow-white/20' : 'border-transparent hover:scale-110 hover:border-white/50'} transition-all`}
                                                ...
                                                <button
                                                  onClick={() => setActiveColorPicker(categoryId)}
                                                  className="opacity-0 group-hover/group:opacity-100 transition-all w-3 h-3 rounded-full border border-white/20 hover:border-white/50 shadow-sm mr-2 shrink-0"
                                                  style={{ backgroundColor: UI_PALETTE[organizedGroups[categoryId].color as ChromeColor]?.hex || uiColor.hex }}
                                                  title="Change color"
                                                />
                                              )}
                                            </>
                                        ...
                                        <ChevronDown size={14} className={`text-slate-500 transition-transform duration-300 ${isCollapsed ? 'rotate-180' : ''} shrink-0`} />
                                      </div>
...
function TabRow({ 
  ...
  return (
    <div className={`flex items-center gap-3 py-2.5 px-4 transition-all group relative border-l-[3px] border-transparent ${
